...
describe("IccCodeConnectAdapter — content (IRC 2021 fixture)", () => {
  ...
  it("lifts model-code cross-references out of section prose", async () => {
    ...
    // "Table R301.2(1)", "Section R302.2", "Section R301" are cited in fixture prose
    expect(labels).toContain("R301.2(1)");
    expect(labels).toContain("R302.2");
    expect(labels).toContain("R301");
    const types = new Set(
  ...
  it("fetch() returns a JSON body; an unknown book yields an empty body", async () => {
    const raw = await mockAdapter.fetch(fixtureReference);
 ...
 * the `/v1/books` endpoint. `tokenIssues` counts how many times the OAuth
...
class StubFetch extends RespectfulFetch {
  ...
  override async fetch(url: string, init: Record<string, unknown> = {}) {
    ...
    return fakeResponse(200, { collections: ICC_CODE_CONNECT_FIXTURES.books }) as never;
  }
}
...
describe("CodeConnectClient — OAuth2 client-credentials flow", () => {
  ...
  it("fetches a token then sends it as a bearer header on API calls", async () => {
    ...
    const tokenCall = http.calls.find((c) => c.url.includes("/auth/token"));
    expect(tokenCall?.init.method).toBe("POST");
    expect(String(tokenCall?.init.body)).toContain(
      "grant_type=client_credentials",
    );
    expect(String(tokenCall?.init.body)).toContain("scope=content-read");

    const apiCall = http.calls.find((c) => c.url.includes("/v1/books"));
  ...
  it("surfaces a non-2xx API response as a CodeConnectError", async () => {
    ...
    await expect(client.listBooks()).rejects.toBeInstanceOf(CodeConnectError);
  });
});
