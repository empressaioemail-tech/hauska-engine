...
export function emitDxfContours(
  ...
  for (const feature of featureCollection.features) {
    ...
    for (const polygon of polygons) {
      for (const ring of polygon) {
        ...
        lines.push("0", "LWPOLYLINE", "8", "TERRAIN_CONTOURS", "90", `${ring.length}`, "70", "1", "38", `${feature.properties.elevationMeters}`);
        for (const [lng, lat] of ring) {
          lines.push("10", `${(lng - bbox.westLng) * metersPerLng}`, "20", `${(lat - bbox.southLat) * 111_320}`);
        }
        polylineCount++;
      }
    }
  ...
  lines.push("0", "ENDSEC", "0", "EOF");
  return { bytes: text.encode(`${lines.join("\n")}\n`), polylineCount };
}
