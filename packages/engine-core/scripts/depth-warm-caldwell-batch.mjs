#!/usr/bin/env node
/**
 * depth-warm-caldwell-batch.mjs — R4 batch warm→verify→promote with cost instrumentation.
 *
 ...
 *     pnpm --filter @hauska-engine/engine-core run depth-warm-caldwell-batch -- \
 ...
 * --place-type-cohort: only RLD/RMD/RHD (descriptor rows); excludes PDD honest no-setback-row noise.
 */
...
import { LOCKHART_CITY_BBOX } from "../src/road-intake/fetch-overpass-bbox.ts";
import { TxgioDatabaseParcelGeometryResolver } from "../src/parcel-terrain/parcel-geometry-resolver.ts";

const COUNTY_FIPS = "48055";
const descriptor = caldwellDescriptor;
...
const cityBbox = LOCKHART_CITY_BBOX;
...
const costJson = {
  event: "RECIPE-PROOF-48055-depth-cost.done",
  ...
  sampleOutcomes,
  wdll9Note: {
    parcel: "48055 resolvable RLD/RMD/RHD cohort",
    status: "PARTIAL",
    detail:
